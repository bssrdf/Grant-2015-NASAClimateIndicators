...
According to the Intergovernmental Panel on Climate Change (IPCC), since 1980 annual losses from extreme weather events have ranged from several billion to over 200 billion dollars.  Weather-related disasters have also caused a significant loss of human life over the past century, including nearly 500,000 deaths from the Bhola Cyclone in Bangladesh (1970), over 1,000,000 deaths from the 1931 central China floods, and 70,000 deaths from the European heat wave of 2003.  Consequently, trends in the characteristics of these extreme weather events will have direct socioeconomic impact that must be understood as part of any climate assessment.
...
The assessment of extreme weather characteristics will augment existing studies that have only targeted specific types of extreme weather event or have only dealt with singular datasets.  This project instead proposed the development of a comprehensive catalogue of past events and their characteristics.  A key advantage from consideration of multiple event types under a single umbrella allows for analysis and identification of connections between these phenomena.  For instance, atmospheric blocking in the Eastern Pacific is responsible for redirecting atmospheric rivers, and can in turn lead to a modification of precipitation patterns.
...
Key characteristics targeted by this proposal include seasonal counts, wind intensity, radius of maximum wind, precipitation intensity, total overland precipitation, genesis locations, and spatial distribution of tracks.
...
Key sectors affected by extratropical cyclones include energy, transportation, urban environments and human health.  Infrastructural vulnerabilities to extreme winter snowfall remain persistent issues, but the effects of winter weather can also affect energy demand and human health.  Within the US, extratropical cyclones primarily affect the Northeast, Great Plains and Midwest regions.
...
\paragraph{Extreme Heat Events (EHEs):}  Heat waves have led to tens of thousands of deaths in the past decade alone, including roughly 40,000 casualties from the 2003 European heat wave \citep{bouchama20042003}, 220 deaths from the 2006 North American heat wave and over 15,000 casualties from the 2010 Russian heat wave.  In 2013, over 760 people died as a consequence of a long-running heat wave in Britain \citep{upi2013article}.  Additional losses from the North American heat wave were caused by drought conditions that spurred widespread forest fires and large-scale convective storm events including the 2012 North American Derecho.  The economic cost of these events has been estimated to be in the tens of billions of dollars.

Various measures of extreme heat have been used. For heat waves, often a percentile threshold is used like 99\% \cite{gershunov2009great}, 97.5\% (\cite{meehl2004more}; with other conditions), 95\% \cite{gosling2007climate}, 90\% \cite{beniston2004heatwave}. Other definitions exist, some with, some without a minimum duration; there is no set definition \cite{grotjahn2008composite, grotjahn2015northamerican}. The WCP and International CLIVAR sponsored development 27 indices by ETCCDMI committee (see: \url{http://etccdi.pacificclimate.org/list_27_indices.shtml}). ETCCDMI indices maxima in monthly periods, percentile exceedances, tropical nights (daily minimum temperature $>$20C).
The NCA uses a variety of measures of extremes: Hard thresholds (T$>$90F, T$>$95F, T$<$32F, in various regions), return period values (hottest once in 20 years), and highest percentages (top 5\%, hottest 2\% of daily max and min temperature). These are all easy to understand measures tailored to the NCA’s broad audience, so we propose to track similar quantities. Tracking changes in the thresholds in thresholds has limited value if the distribution of the upper tail is also changing \cite{vautard2013simulation} so we shall explore these properties as useful quantities for scientific users of these algorithms. 
...
Key characteristics include duration of heat events, percentile, apparent temperatures during heat events, overnight recovery temperatures and spatial extent.
...
Various measures of extreme precipitation events exist. Several types of grouping exist based first on the time interval over which the extreme precipitation occurs: a flash flood from a thunderstorm to a seasonal accumulation. For example, a robust ETCCDMI index for climate model simulations \cite{sillmann2008indices} is the maximum 5-day interval precipitation during a month (RX5day); \cite{sillmann2009present} claim that extreme values of this index are a useful indicator for large scale flooding events over Europe. Other ETCCDMI indices target 1 day maxima up to annual amounts exceeding 95\%.  The NCA uses percentiles (top 2\% of daily rainfall in a year, heaviest 1\% of all daily events each year), return period threshold (the 2-day total that occurs once in a 5 year period).
...
This proposal aims to produce both tabulated and graphical data representations of the results (event severity, location, and duration).  Maps of changing extreme weather density over North America, along with informational labels identifying changes in characteristics that will affect key regions and sectors and an indication of statistical significance.  Maps will focus on climatically-consistent regions, specifically the 344 NOAA climate devisions.  Box-and-whisker plots will be produced showing changes in characteristics when breaking the sample period into two (for example, 1979-1997 and 1998-2016).  Time series of individual characteristics by region will be produced, as appropriate.
